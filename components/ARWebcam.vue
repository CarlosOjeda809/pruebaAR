...
const markerImg = ref(null)
onMounted(() => {
  if (process.client) {
    markerImg.value = new Image()
    markerImg.value.src = '/public/sprites/fondocolorido.png'
  }
})
</script>
    ...
    <div class="ar-container">
        ...
        <div v-if="showAR" class="ar-scene-container">
            <a-scene embedded arjs="sourceType: webcam; trackingMethod: best; debugUIEnabled: false;"
                ...
                <a-entity gltf-model="/sprites/SamsungS10.gltf" scale="0.1 0.1 0.1"
        ...
        <button v-if="!showAR && !loading" @click="startAR"
            class="bg-green-600 shadow-md text-white px-4 py-2 text-lg rounded-md">
            Iniciar AR sin marcador
        </button>

        <div v-if="!showAR && !loading" class="marker-info mt-4">
            <p class="text-sm text-gray-600">
                Nota: Puedes imprimir un marcador para mejor posicionamiento o usar sin marcador.
            </p>
        </div>
    </div>
</template>
